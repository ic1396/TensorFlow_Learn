...
def generate (sample_size, num_classes, diff, regression=False):
    ...
    if regression == False:   # one-hot 编码，将 0 转换成 10
...
err = tf.count_nonzero(a1 - b1) # 两个数组相减，不为 0 的就是错误个数

cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=input_lables, logits=output)
loss = tf.reduce_mean(cross_entropy)    # 对交叉熵取均值

optimizer = tf.train.AdamOptimizer(0.04)   #收敛快，可动态调节梯度
